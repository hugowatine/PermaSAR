...
batch_col2cor.py
--------------------
Script pour lancer automatiquement col2cor.py sur tous les dossiers int_YYYYMMDD_YYYYMMDD.

Usage:
    batch_col2cor.py --path=<path> [--nproc=<n>] [--list_int=<file>]
    batch_col2cor.py -h | --help
...
def process_subdir(flatsim_script, base_dir, subdir):
    ...
    for f in os.listdir(full_path):
        if f.startswith("col") and f.endswith("mask_nomodel_8rlks.int"):
        #if f.startswith("col") and f.endswith("mask_8rlks.int"):
...
def main():
    ...
    flatsim_script = os.path.expanduser("~/PermaSAR/Tools/col2cor.py")
