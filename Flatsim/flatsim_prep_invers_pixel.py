...
if (arguments["--Bc"] == None):
   ...
   a, b = arguments["--Bc"].split(",")

   btc = None if a.strip() == "_" else float(a)
   bpc = None if b.strip() == "_" else float(b)

   if btc is None and bpc is None:
      print("Error: at least one value must be provided in --Bc.")
      sys.exit()
...
if (arguments["--sigma"] == None) &  (arguments["--Bc"] == None): 
    ...
    wf.close()
     ...
     for i in range((kmax)):
        if btc is not None and bpc is not None:  
          deltat = btc/(abs(bt[im==date_1[i]] - bt[im==date_2[i]]))
          deltap = (abs(bp[im==date_1[i]] - bp[im==date_2[i]]))/bpc
          weight[i] = (float(np.exp(-deltap)) + float(np.exp(-deltat)))/2
          # weight[i] = (float(np.exp(-deltap)) * float(np.exp(-deltat)))/2
        elif btc is not None and bpc is None:
           deltat = btc/(abs(bt[im==date_1[i]] - bt[im==date_2[i]]))
           weight[i] = float(np.exp(-deltat))
        else:
           deltap = (abs(bp[im==date_1[i]] - bp[im==date_2[i]]))/bpc
           weight[i] = float(np.exp(-deltap))
        # Impose min weight to 0.1
        weight[i] = max(weight[i], 0.1)
     wf = open(os.path.join(tsdir, "list_pair"), "w")
     for i in range((kmax)):
          wf.write("%i %i %.6f\n" % (date_1[i], date_2[i], weight[i]))
     wf.close()
