...
insarx, insary = np.meshgrid(insarxx,insaryy)
...
#print(shape(index))
print(f"Max index: {np.max(index)}, Size of los: {los.shape[0]}")

print(f"los size: {los.shape}")
print(f"insarx size: {insarx.shape}")
print(f"insary size: {insary.shape}")
print(f"insarxp size: {insarxp.shape}")
print(f"insaryp size: {insaryp.shape}")
...
iilos = np.delete(los, index)
iix = np.delete(insarx, index)
iiy = np.delete(insary, index)
iixp = np.delete(insarxp, index)
iiyp = np.delete(insaryp, index)
...
cax2 = ax2.imshow(insar,cmap=cmap,vmax=vmax,vmin=vmin,alpha=0.6, interpolation='nearest')
...
cax3 = ax3.imshow(insar,cmap=cmap,vmax=vmax,vmin=vmin,extent=None,alpha=0.6, interpolation='nearest')
