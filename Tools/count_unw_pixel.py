...
Options:
  ...
  --int_list=<file>     Text file containing list of interferograms dates in two (or 3) colums, $data1 $date2 ($weight) [default: list_pair]
...
try:
    ...
    date_1, date_2, weight = np.loadtxt(int_list, comments="#", unpack=True, dtype='i,i,f')
    doweight = True
...
for k in range(kmax):
     ...
     for j in range(0,nlines): # maps[i,j]
         ...
         if doweight == False:
            ...
            summ[j,index] +=  Bt*weight[k]
            count[j,index] +=  weight[k]
