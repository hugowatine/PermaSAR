...
[--dateslim=<values_min,value_max>]  [--nproc=<nb_cores>] [--filter=<HP/LP>] [--fwindsize=<value>] 
...
def estim_ramp(los,los_clean,topo_clean,az,rg,order,sigma,nfit,ivar,l,ax_dphi):
      ...
      gauss_filter = np.zeros((new_lines,new_cols))
      ...
      if arguments["--filter"] != None:
            los_filter = np.copy(los_clean) 
            weight_filter = 0*np.copy(los_clean)+1 # => un Array de 1 
            index = np.isnan(los_clean)
            # On remplace les nan par 0
            los_filter[index] = 0.
            weight_filter[index] = 0.
            if arguments["--filter"] == 'HP':
                gauss_filter = ndimage.gaussian_filter(los_filter, int(arguments["--fwindsize"]))/ndimage.gaussian_filter(weight_filter, int(arguments["--fwindsize"]))
            if arguments["--filter"] == 'LP':
                gauss_filter = los.reshape(new_lines,new_cols) - ndimage.gaussian_filter(los_filter, int(arguments["--fwindsize"]))/ndimage.gaussian_filter(weight_filter, int(arguments["--fwindsize"]))
            gauss_filter[index] = float('NaN')

      #print(los.reshape(new_lines,new_cols))
      #print(ramp)
      #print(topo)
      #print(gauss_filter)
      flata = los_clean.reshape(new_lines,new_cols) - ramp - topo #- gauss_filter
      #print(flata)
      #print(len(flata))
      #print(flata[~np.isnan(flata)])
      #print(len(flata[~np.isnan(flata)]))
      rms = np.sqrt(np.nanmean(res**2))
      
      if arguments["--filter"] != None:
          rms = np.sqrt(np.nanmean(flata**2))
      logger.info('RMS dates %i: %f'%(idates[l], rms))
      ...
      if True:
        ...
        vmax, vmin = np.nanpercentile(flata,98), np.nanpercentile(flata,2)

        # Afficher la bande filtrée
        cax = ax.imshow(flata, cmap=cmap, vmin=-4, vmax=4, interpolation='nearest')
        ...
        # Afficher la bande filtrée
        cax = ax.imshow(los.reshape(new_lines,new_cols), cmap=cmap, vmin=-4, vmax=4, interpolation='nearest')
        ax.set_title('Filtered Band')
...
def empirical_cor(l):
  ...
  if np.nansum(maps[:,:,l]) != 0:
    ...
    # clean maps
    if arguments["--filter"] != None:
        # Remplace les valeurs hors du masque par NaN
        los_clean = np.where(mask, maps_temp, 'NaN')
    else:
...
# create new cube
if flat>0 or save_depl_cumule == 'yes':
