...
#
##min_value = np.nanmin(cubei)
##max_value = np.nanmax(cubei)
##print(f"Valeur min : {min_value}, Valeur max : {max_value}")
#
...
cube = np.zeros((nlines, ncol, N), dtype=np.float32)
for band_index in range(1, N + 1):
    band = ds.GetRasterBand(band_index)
    cube[:, :, band_index - 1] = band.ReadAsArray()
logger.info('Load time series cube: {0}, with length: {1}'.format(arguments["--cube"], len(cube)))
kk = np.flatnonzero(np.logical_or(cube==9990, cube==9999))
cube[kk] = float('NaN')
maps_temp = cube

print(maps_temp.shape)  # Pour vérifier si maps_temp est 3D et quelle est sa forme
...
#    print(f"Traitement de la bande {band_index + 1}/{N}...")
#    start_time = time.time()
#    print(band_index)
#    # Lire la bande actuelle
#    band = maps_temp[:, :, band_index]
#
#    # Appliquer un traitement à la bande
#    min_b = np.nanmin(band)
#    max_b = np.nanmax(band)
#    rms_band = np.sqrt(np.nanmean(band**2))
#    
#    l_rms.append(rms_band)
#    l_max.append(max_b)
#    l_min.append(min_b)
#
#    elapsed_time = time.time() - start_time
#    print(f"Temps de traitement pour la bande {band_index + 1}: {elapsed_time:.2f} secondes")
#
### Résumé des résultats
#print('------')
#print(f"data ; min,max : {min(l_min)}, {max(l_max)}")
#print(f"rms ; min,max,mean : {min(l_rms)}, {max(l_rms)}, {np.mean(l_rms)}")
#print('------')
###
      ...
      if order==0:
        ...
        if arguments["--topofile"] is None:
            ...
            if (ivar==0 and nfit==1):
                ...
                ramp = np.dot(G[:,:(nparam-3)],pars[:nparam-3]).reshape(new_lines,new_cols)
                topo = np.dot(G[:,(nparam-3):],pars[(nparam-3):]).reshape(new_lines,new_cols)
      print("res : ", np.nanmean(res))
      print("res**2 :" , np.nanmean(res **2))
